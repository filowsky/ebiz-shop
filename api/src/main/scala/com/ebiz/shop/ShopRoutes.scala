...
import org.http4s.{EntityDecoder, EntityEncoder, HttpRoutes, ResponseCookie, SameSite, Uri}
    ...
    HttpRoutes.of[F] {
      case req@POST -> Root / "auth" =>
        for {
          ...
          withCookie = resp
            .addCookie(ResponseCookie(name = "shop_auth", content = authCookie, secure = true, sameSite = SameSite.None))
        } yield withCookie
